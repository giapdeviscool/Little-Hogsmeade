...
import { branchPerformance, kpis } from '../../_mock/dashboard.mock'
  ...
  return (
    <>
      <div className="flex items-start justify-between gap-6">
        <div>
          <p className="mb-1 text-sm font-medium text-muted">Chain Owner View · 18 Tháng 5, 2026</p>
      ...
      <section className="mt-8 grid grid-cols-4 gap-5">
        {kpis.map(([label, value, delta, good], index) => (
          <Card key={label} className="min-h-[234px] p-6">
            <div className="mb-6 flex items-center justify-between">
              <span className="grid h-11 w-11 place-items-center rounded-[13px] bg-white text-xl">{['$', '▣', '♧', '▤'][index]}</span>
              <span className={cn('rounded-full px-3 py-1 text-xs font-bold', good ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700')}>⌁ {delta}</span>
            </div>
            <p className="text-sm font-medium text-muted">{label}</p>
            <strong className="mt-1 block text-[31px] tracking-[-0.04em]">{value}</strong>
            <Sparkline />
          </Card>
        ))}
      </section>
      ...
      <section className="mt-8 grid grid-cols-[1.25fr_.8fr] gap-6">
        <Card className="p-6">
          ...
          {branchPerformance.map(([branch, value]) => (
            <div key={branch} className="mt-4 flex items-center gap-4 rounded-[14px] bg-white p-3">
              <span className="w-28 text-sm font-semibold">{branch}</span>
        ...
        <Card className="p-6">
          ...
          {products.slice(0, 4).map(([name, price, img], i) => (
            <div key={name} className="mt-4 flex items-center gap-4 rounded-[14px] bg-white p-3">
              <img src={img} alt="" className="h-12 w-12 rounded-[12px] object-cover" />
              <div className="flex-1"><strong className="block text-sm">{name}</strong><span className="text-xs text-muted">{price}</span></div>
