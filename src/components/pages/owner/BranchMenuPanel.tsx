...
export function BranchMenuPanel({
  ...
  const handleSaveBranchMenu = useCallback(
    async (branchId: string, items: any[]) => {
      try {
  ...
  return (
    <Tabs
      value={activeOuterTab}
      onValueChange={setActiveOuterTab}
      className="w-full"
    >
      {/* Outer tab bar */}
                ...
                <TabsList className="h-auto gap-0 bg-transparent min-w-max px-0">
                  ...
                  {branchTabs.map((bt) => (
                    <TabsTrigger
                      key={bt.id}
                      value={bt.id}
                      className={BranchTabTriggerStyle}
                    >
              ...
              {loadingBranch && !branchMenus[bt.id] ? (
                ...
                <div className="rounded-xl border border-dashed border-line py-16 text-center">
                  ...
                  <p className="mt-1 text-sm text-muted/70">
                    Hãy đồng bộ từ Base Menu trước.
                  </p>
                </div>
              )}
            </TabsContent>
